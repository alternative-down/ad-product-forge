// Thread helpers for agent-home-metrics — libsql I/O and step interval computation.
...
import { mergeToolLogMessages, buildThreadToolInvocationParts, type ToolLogMessage } from './agent-home-metrics-tool-helpers';
...
export async function readLatestThreadDetails(
  ...
  agentId: string,
): Promise<{ preview: string | null; toolBadge: { icon: string; label: string } | null }> {
  try {

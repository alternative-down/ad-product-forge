...
export function createAgentLongTermMemory(input: {
  ...
  async function writeCheckpointPackage(payload: CheckpointedOmCheckpointPackageInput) {
    ...
    try {
      await prepareTempPackageDirectory(tempPackagePath);
      await writeCheckpointFiles(tempPackagePath, payload);
      await commitCheckpointPackage(packagePath, tempPackagePath);
      ...
      await cleanupTempPackage(tempPackagePath);

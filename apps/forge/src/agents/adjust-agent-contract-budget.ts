...
export async function adjustAgentContractBudget(
  ...
  try {
    ...
    forgeDebug({ scope: "agents-adjust-agent-contract-budget.ts", level: "error", message: "agents-adjust-agent-contract-budget.ts: unhandled error", error: err instanceof Error ? err.message : String(err) });
    throw err;
  }

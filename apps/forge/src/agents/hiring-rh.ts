...
export async function generateHiredAgentInstructions(
  ...
  const agent = new Agent({
    ...
    instructions: [
      ...
      '- Do NOT block hiring because external integrations, provider credentials, or communication accounts are not configured yet.',
      '- Integrations and provider setup are optional follow-up configuration, not a hiring prerequisite.',
...
function buildHiringPrompt(input: {
  ...
  const sections = [
    ...
    'Do not require provider credentials, external integrations, communication channel setup, or system integrations before approving the hire.',
    'Those integrations can be configured later and must not block hireAgent.',

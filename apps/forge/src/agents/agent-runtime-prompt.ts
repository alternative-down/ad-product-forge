...
export function buildAgentSystemPrompt(input: {
  ...
  const sections = [
    ...
    [
      ...
      '- Do not loop on working memory. If a memory update is needed, make one concise update and continue with the real work. Do not spend repeated steps narrating that you will update memory, checking whether it happened, or trying to call hidden internal mechanisms again and again.',
      '- Do not get stuck reasoning about internal system behavior such as whether a memory update, internal helper, or hidden runtime action was executed. If you already issued the relevant action once, stop re-litigating it and continue with the next real step.',
      '- If you notice you are repeating the same intention about memory, stopping, or internal tool execution without moving the work forward, break the loop immediately and return to concrete work, a concrete tool call, or a real stop decision.',

...
export function buildAgentSystemPrompt(input: {
  ...
  const sections = [
    ...
    [
      ...
      '- Update working memory as soon as something materially changes in your active rules, domain expansion, or current objectives. Do not wait until the end of the run.',
      '- Use working memory domain expansion to capture the practical constitution of your area inside the limits of your role: what belongs to your function, how you usually execute the work, what kinds of activities are properly yours, and where the scope edges are in practice.',
      '- Domain expansion is for deepening your own role, not for stretching it. Use it to record the lived shape of the role as you understand it through real work without crossing into another role or inventing broader authority.',
      ...
      '- Use working memory only for compact continuity about standing rules, domain expansion, and current objectives.',
      '- Do not use working memory for notes, facts, logs, detailed findings, or task tracking that already belongs in workspace files.',
      ...
      '- Use workspace records for detailed dumps and evolving documentation. Use working memory only for compact rules, domain expansion, and objectives. They are complementary and should not contain the exact same material.',
      '- Record things in the workspace when they may matter later: facts, open questions, partial conclusions, hypotheses, observations, decisions, lessons learned, plans to revisit, detailed task tracking, and detailed context that should not live in working memory.',
      ...
      '- Use workspace notes and working memory as tools for active analysis, not just storage. Read them, reflect on them, infer what they imply, identify what changed, and turn those conclusions into next actions, follow-ups, or stronger operating guidance.',
    ...
    [
      ...
      '- In working memory, keep only rules, domain expansion, and objectives concise and well structured. Prefer dense text over verbose narrative.',
      '- Working memory should help you preserve continuity about role behavior and current direction, not become a dump of everything that happened.',

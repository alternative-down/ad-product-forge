...
>(
  ...
  try {
    ...
    forgeDebug({ scope: 'create-forge-agent', level: 'error', message: '[create-forge-agent] createInternalAgentRuntime failed', context: { error: err instanceof Error ? err.message : String(err), agentId: config.id }});
    throw err;
  }
}

...
const _m = vi.hoisted(() => {
  ...
  const resolveBundledSkillRoot = vi.fn((name: string) =>
    Promise.resolve(`/base/_bundled/${name}`),
  );
...
vi.mock('./bundled-workspace-skills', async () => {
  const Real = await vi.importActual<typeof import('./bundled-workspace-skills')>(
...
vi.mock('./workspace-skill-paths', async () => {
  const Real =
    await vi.importActual<typeof import('./workspace-skill-paths')>('./workspace-skill-paths');
  return {
...
(gs as any).BUNDLED_SKILL_DIRECTORY_NAMES = (
  (await vi.importActual('./bundled-workspace-skills')) as any
...
beforeEach(() => {
  ...
  accessFn.mockImplementation(async (p: string) => {
    ...
    if (!fileStore.has(key) && !dirStore.has(key))
      throw Object.assign(new Error('ENOENT'), { code: 'ENOENT' });
  });
  statFn.mockImplementation(async (p: string) => {
    const key = String(p).replace(/\/$/, '');
    if (fileStore.has(key))
      return { isDirectory: () => false, isFile: () => true, mtimeMs: 1700000000000 } as Awaited<
        ReturnType<(typeof import('node:fs/promises'))['stat']>
      >;
    if (dirStore.has(key))
      return { isDirectory: () => true, isFile: () => false, mtimeMs: 1700000000000 } as Awaited<
        ReturnType<(typeof import('node:fs/promises'))['stat']>
      >;
    throw Object.assign(new Error('ENOENT'), { code: 'ENOENT' });
  });
  readdirFn.mockImplementation(async (p: string) => {
    ...
    return entries.map((name) => {
      ...
      const entry: import('node:fs').Dirent = {
        name,
        isDirectory: () => isDir,
        isFile: () => !isDir,
        isBlockDevice: () => false,
  ...
  resolveBundledSkillRoot
    .mockReset()
    .mockImplementation(async (name: string) => `/base/_bundled/${name}`);
...
describe('listGlobalSkills', () => {
  ...
  it('returns bundled skills when no custom skills exist', async () => {
    ...
    seedBundledSkill('coolify-api');
    seedBundledSkill('skills-creator');
  ...
  it('bundled skill overrides custom skill with same name', async () => {
    ...
    const skill = (await listGlobalSkills('/base')).find(
      (s: { skillName: string }) => s.skillName === 'github-api',
    );
  ...
  it('returns fileCount from countSkillFiles', async () => {
    ...
    const result = await listGlobalSkills('/base');
    const githubApi = result.find(
      (s: { skillName: string; fileCount?: number }) => s.skillName === 'github-api',
    );
...
describe('installGlobalSkillsFromZip', () => {
  ...
  it('creates skillsRoot directory', async () => {
    ...
    expect(mkdirFn).toHaveBeenCalledWith(expect.stringContaining('_system/skills'), {
      recursive: true,
    });
  });
  ...
  it('returns sorted skill names', async () => {
    ...
    const result = await installGlobalSkillsFromZip({
      zipBase64: 'ZmFrZQ==',
  ...
  it('throws when archive has no files', async () => {
    unzipSync.mockReturnValue({});
    await expect(
      installGlobalSkillsFromZip({ zipBase64: 'ZmFrZQ==', workspaceBasePath: '/base' }),
  ...
  it('throws when skill name is reserved by bundled skill', async () => {
    ...
    await expect(
      installGlobalSkillsFromZip({ zipBase64: 'ZmFrZQ==', workspaceBasePath: '/base' }),
    ).rejects.toThrow('Skill name is reserved by a bundled skill: github-api');
  });
  ...
  it('skips directory entries', async () => {
    unzipSync.mockReturnValue({
      ...
      'my-skill/SKILL.md': new Uint8Array(0),
    });
    const result = await installGlobalSkillsFromZip({
      zipBase64: 'ZmFrZQ==',
      workspaceBasePath: '/base',
...
describe('deleteGlobalSkill', () => {
  ...
  it('throws when skillName contains path traversal', async () => {
    await expect(
      deleteGlobalSkill({ workspaceBasePath: '/base', skillName: '../etc/passwd' }),
  ...
  it('whitespace skillName does not reject and resolves to undefined', async () => {
    await expect(
      deleteGlobalSkill({ workspaceBasePath: '/base', skillName: '   ' }),
...
describe('installGlobalSkillToAgentWorkspace', () => {
  ...
  it('throws when custom skill not found', async () => {
    ...
    ).rejects.toThrow('Global skill not found');
  });
...
describe('publishAgentWorkspaceSkillToGlobalCatalog', () => {
  ...
  it('throws when skill already exists as bundled skill', async () => {
    ...
    ).rejects.toThrow('Skill name is reserved by a bundled skill');
  });
  ...
  it('copies skill to global catalog when not already installed', async () => {
    ...
    fileStore.set(
      '/base/agent-42/workspace/skills/my-tool/SKILL.md',
      '---\ndescription: My tool\n---\n',
    );

...
describe('createAgentSkillTools', () => {
  ...
  describe('load_workspace_skill.execute', () => {
    ...
    it('loads skill markdown and support files', async () => {
      ...
      const result = (await tools.load_workspace_skill.execute(
        { skillName: 'my-skill' },
        {} as any,
      )) as any;
    ...
    it('recurses into subdirectories', async () => {
      ...
      mockFs.readFile.mockResolvedValueOnce('# Skill').mockResolvedValueOnce('{"nested":true}');
      const tools = createAgentSkillTools({
        db: mockDb,
        workspaceBasePath: '/base',
        agentId: 'agent-42',
    ...
    it('logs warning and returns null for unreadable support files', async () => {
      ...
      mockFs.readFile.mockResolvedValueOnce('# Skill').mockRejectedValueOnce(new Error('ENOENT'));
      const tools = createAgentSkillTools({
        db: mockDb,
        workspaceBasePath: '/base',
        agentId: 'agent-42',
  ...
  describe('publish_skill_to_catalog.execute', () => {
    ...
    it('calls publishAgentWorkspaceSkillToGlobalCatalog and returns success', async () => {
      ...
      const result = (await tools.publish_skill_to_catalog.execute(
        { skillName: 'my-skill' },
        {} as any,
      )) as any;

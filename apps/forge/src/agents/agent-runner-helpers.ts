...
function isNoActionNeeded(text: string): boolean {
  return text.trimStart().startsWith(NO_ACTION_NEEDED_PREFIX);
}

/** Returns true if the given text starts with the STOP_AND_IDLE marker. */
function isStopAndIdle(text: string): boolean {
  return text.trimStart().startsWith(STOP_AND_IDLE_PREFIX);
}

/** Returns the control directive (stop | no-action-needed | null) for the given step text. */
function extractControlDirective(text: string): 'stop' | 'no-action-needed' | null {

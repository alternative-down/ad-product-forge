...
const { mockRecordCashOut, mockRecordCashIn, mockGetCurrentBalanceUsd, mockGetContractSpend } = vi.hoisted(() => ({
...
vi.mock('../finance/company-cash-operations', () => ({
  createCompanyCashOperations: vi.fn(() => ({ recordCashOut: mockRecordCashOut, recordCashIn: mockRecordCashIn })),
}));
...
describe('adjustAgentContractBudget', () => {
  ...
  it('throws when no active contract exists', async () => {
    ...
    await expect(adjustAgentContractBudget(db as any, { agentId: 'agent-1', newBudgetUsd: 200 }))
  ...
  it('returns changeType none when budget is unchanged', async () => {
    ...
    const result = await adjustAgentContractBudget(db as any, { agentId: 'agent-1', newBudgetUsd: 100 });
  ...
  it('throws on budget decrease below spent amount', async () => {
    ...
    const db = createMockDb(mockContract({ budgetUsd: 100 }));
    await expect(adjustAgentContractBudget(db as any, { agentId: 'agent-1', newBudgetUsd: 50 }))
      .rejects.toThrow(/Cannot reduce budget below spent amount/);
  });

  it('returns changeType increase with positive delta', async () => {
    const db = createMockDb(mockContract({ budgetUsd: 100 }));
    const result = await adjustAgentContractBudget(db as any, { agentId: 'agent-1', newBudgetUsd: 150 });
  ...
  it('returns changeType decrease with negative delta', async () => {
    const db = createMockDb(mockContract({ budgetUsd: 100 }));
    const result = await adjustAgentContractBudget(db as any, { agentId: 'agent-1', newBudgetUsd: 80 });
  ...
  it('returns object with agentId and contractId', async () => {
    ...
    const result = await adjustAgentContractBudget(db as any, { agentId: 'agent-xyz', newBudgetUsd: 120 });

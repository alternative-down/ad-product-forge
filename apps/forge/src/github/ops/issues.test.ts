...
describe('createIssuesOps — getIssue', () => {
  ...
  it('getIssue calls correct endpoint with owner from getDefaultOwner', async () => {
    ...
    const ctx = makeCtx();
    const issues = createIssuesOps(ctx);
    const result = await issues.getIssue('agent-1', { owner: 'my-org', repositoryName: 'my-repo', issueNumber: 10 });
    // getDefaultOwner in mock always returns 'acme' (ignores input)
    expect(octokitMock.request).toHaveBeenCalledWith('GET /repos/{owner}/{repo}/issues/{issue_number}', {
      owner: 'acme',
      repo: 'my-repo',
      issue_number: 10,
    });
  ...
  it('getIssue delegates to toIssueDetails', async () => {
    const { createIssuesOps } = await import('./issues.js');
    const mockData = { id: 99, number: 5, title: 'Feature', body: 'Long description', state: 'open' };
...
describe('createIssuesOps — createIssue', () => {
  ...
  it('createIssue POSTs with correct fields', async () => {
    ...
    expect(octokitMock.request).toHaveBeenCalledWith('POST /repos/{owner}/{repo}/issues', expect.objectContaining({
      title: 'New issue',
      body: 'Details',
    }));
  });
  ...
  it('createIssue passes labels and assignees', async () => {
    const { createIssuesOps } = await import('./issues.js');
    octokitMock.request.mockResolvedValue({ data: { id: 1, number: 1, title: 'T', state: 'open' } });
    const ctx = makeCtx();
    const issues = createIssuesOps(ctx);
    await issues.createIssue('agent-1', {
      repositoryName: 'repo',
...
describe('createIssuesOps — updateIssue', () => {
  ...
  it('updateIssue PATCHes with provided fields', async () => {
    const { createIssuesOps } = await import('./issues.js');
    octokitMock.request.mockResolvedValue({ data: { id: 1, number: 5, title: 'Updated', state: 'open' } });
    const ctx = makeCtx();
    const issues = createIssuesOps(ctx);
    await issues.updateIssue('agent-1', { repositoryName: 'repo', issueNumber: 5, title: 'Updated', state: 'closed' });
    expect(octokitMock.request).toHaveBeenCalledWith('PATCH /repos/{owner}/{repo}/issues/{issue_number}', expect.objectContaining({
      issue_number: 5,
      title: 'Updated',
      state: 'closed',
    }));
  ...
  it('updateIssue omits undefined fields from PATCH body', async () => {
    ...
    const ctx = makeCtx();
    const issues = createIssuesOps(ctx);
    await issues.updateIssue('agent-1', { repositoryName: 'repo', issueNumber: 5, body: 'New body' });
  ...
  it('updateIssue delegates to toIssueDetails', async () => {
    ...
    const ctx = makeCtx();
    const issues = createIssuesOps(ctx);
    const result = await issues.updateIssue('agent-1', { repositoryName: 'repo', issueNumber: 10, title: 'Patched' });
    expect(ctx.toIssueDetails).toHaveBeenCalledWith(mockData);
  });
});
...
describe('createIssuesOps — listIssueComments', () => {
  ...
  it('listIssueComments calls GET with correct params', async () => {
    ...
    const ctx = makeCtx();
    const issues = createIssuesOps(ctx);
    await issues.listIssueComments('agent-1', { owner: 'org', repositoryName: 'repo', issueNumber: 7 });
    expect(octokitMock.request).toHaveBeenCalledWith('GET /repos/{owner}/{repo}/issues/{issue_number}/comments', {
      owner: 'acme',
      repo: 'repo',
      issue_number: 7,
      per_page: 100,
  ...
  it('listIssueComments returns formatted comment objects', async () => {
    ...
    const ctx = makeCtx();
    const issues = createIssuesOps(ctx);
    const result = await issues.listIssueComments('agent-1', { repositoryName: 'repo', issueNumber: 7 });
...
describe('createIssuesOps — getIssueComment', () => {
  ...
  it('getIssueComment calls correct endpoint', async () => {
    const { createIssuesOps } = await import('./issues.js');
    octokitMock.request.mockResolvedValue({
      data: { id: 5, html_url: 'https://github.com/org/repo/issues/1#issuecomment-5', body: 'Test', user: { login: 'bob' }, created_at: '2024-01-01', updated_at: '2024-01-02' },
    });
    const ctx = makeCtx();
    const issues = createIssuesOps(ctx);
    const result = await issues.getIssueComment('agent-1', { repositoryName: 'repo', issueNumber: 1, commentId: 5 });
    expect(octokitMock.request).toHaveBeenCalledWith('GET /repos/{owner}/{repo}/issues/comments/{comment_id}', {
      owner: 'acme',
      repo: 'repo',
      issue_number: 1,
      comment_id: 5,
...
describe('createIssuesOps — createIssueComment', () => {
  ...
  it('createIssueComment POSTs with body and returns formatted comment', async () => {
    const { createIssuesOps } = await import('./issues.js');
    octokitMock.request.mockResolvedValue({
      data: { id: 99, html_url: 'https://github.com/org/repo/issues/5#issuecomment-99', body: 'Hello world', user: { login: 'carol' }, created_at: '2024-03-01', updated_at: '2024-03-01' },
    });
    const ctx = makeCtx();
    const issues = createIssuesOps(ctx);
    const result = await issues.createIssueComment('agent-1', { repositoryName: 'repo', issueNumber: 5, body: 'Hello world' });
    expect(octokitMock.request).toHaveBeenCalledWith('POST /repos/{owner}/{repo}/issues/{issue_number}/comments', {
      owner: 'acme',
      repo: 'repo',
      issue_number: 5,
      body: 'Hello world',
...
describe('createIssuesOps — updateIssueComment', () => {
  ...
  it('updateIssueComment PATCHes with new body', async () => {
    const { createIssuesOps } = await import('./issues.js');
    octokitMock.request.mockResolvedValue({
      data: { id: 77, html_url: 'https://github.com/org/repo/issues/1#issuecomment-77', body: 'Updated text', user: { login: 'dave' }, created_at: '2024-01-01', updated_at: '2024-01-05' },
    });
    const ctx = makeCtx();
    const issues = createIssuesOps(ctx);
    const result = await issues.updateIssueComment('agent-1', { repositoryName: 'repo', commentId: 77, body: 'Updated text' });
    // owner comes from getDefaultOwner which returns 'acme' in the mock
    expect(octokitMock.request).toHaveBeenCalledWith('PATCH /repos/{owner}/{repo}/issues/comments/{comment_id}', {
      owner: 'acme',
      repo: 'repo',
      comment_id: 77,
      body: 'Updated text',
...
describe('createIssuesOps — deleteIssueComment', () => {
  ...
  it('deleteIssueComment DELETE returns {success:true}', async () => {
    ...
    const ctx = makeCtx();
    const issues = createIssuesOps(ctx);
    const result = await issues.deleteIssueComment('agent-1', { repositoryName: 'repo', commentId: 55 });
    expect(octokitMock.request).toHaveBeenCalledWith('DELETE /repos/{owner}/{repo}/issues/comments/{comment_id}', {
      owner: 'acme',
      repo: 'repo',
      comment_id: 55,
    });

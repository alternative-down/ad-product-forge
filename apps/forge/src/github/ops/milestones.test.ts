...
describe('createMilestonesOps — createMilestone', () => {
  ...
  it('createMilestone POSTs with correct fields', async () => {
    ...
    await milestones.createMilestone('agent-1', {
      repositoryName: 'repo',
      title: 'v1.0',
      description: 'Release v1.0',
    });
  ...
  it('createMilestone returns formatted milestone', async () => {
    ...
    const ctx = makeCtx();
    const milestones = createMilestonesOps(ctx);
    const result = await milestones.createMilestone('agent-1', {
      repositoryName: 'repo',
      title: 'v3.0',
    });
    expect(result).toEqual({
...
describe('createMilestonesOps — updateMilestone', () => {
  ...
  it('updateMilestone PATCHes with provided fields', async () => {
    ...
    const ctx = makeCtx();
    const milestones = createMilestonesOps(ctx);
    await milestones.updateMilestone('agent-1', {
      repositoryName: 'repo',
      milestoneNumber: 1,
      title: 'Updated title',
      state: 'closed',
  ...
  it('updateMilestone returns formatted milestone', async () => {
    ...
    const ctx = makeCtx();
    const milestones = createMilestonesOps(ctx);
    const result = await milestones.updateMilestone('agent-1', {
      repositoryName: 'repo',
      milestoneNumber: 4,
    });
    expect(result).toEqual({
...
describe('createMilestonesOps — deleteMilestone', () => {
  ...
  it('deleteMilestone DELETE returns {success:true}', async () => {
    ...
    const ctx = makeCtx();
    const milestones = createMilestonesOps(ctx);
    const result = await milestones.deleteMilestone('agent-1', {
      repositoryName: 'repo',
      milestoneNumber: 2,
    });
    expect(octokitMock.request).toHaveBeenCalledWith(

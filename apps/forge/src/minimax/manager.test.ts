...
describe('MiniMaxClient', () => {
  ...
  describe('textToSpeech', () => {
    ...
    it('returns success when data.data.audio is present (nested path)', async () => {
      ...
      setupFetch(makeJsonResponse({
        base_resp: { status_code: 0, status_msg: 'success' },
        data: { audio: "NESTED123" },(test(#876): add unit test coverage for minimax/manager.ts)
      }));
    ...
    it('handles voice settings with only voiceId provided', async () => {
      ...
      expect(body.voice_setting.speed).toBe(1);(test(#876): add unit test coverage for minimax/manager.ts)
    });
  });
  // ── listVoices ───────────────────────────────────────────────────────────
  ...
  describe('createMiniMaxClient', () => {
    it('creates client from apiKey argument', async () => {
      ...
      expect(client.constructor.name).toBe('MiniMaxClient');(test(#876): add unit test coverage for minimax/manager.ts)
    });

...
export function createMicroErpReadModel(db: Database) {
  ...
  async function getCompanyCashBalance() {
    ...
    try {
      ...
      forgeDebug({ scope: "micro-erp-read-model.ts", level: "error", message: "micro-erp-read-model.ts: unhandled error", error: err instanceof Error ? err.message : String(err) });
      throw err;
    }
    return { balanceUsd };
  }

...
export function createWebhookStore(db: Database) {
  async function createRoute(input: {
    ...
    try {
      ...
      forgeDebug({
        ...
        level: 'error',
        message: 'createRoute: db.insert failed',
        context: {
          agentId: input.agentId,
  ...
  async function listRoutesByAgent(agentId: string): Promise<WebhookRoute[]> {
    try {
      ...
      return _rows as unknown as any[] as WebhookRoute[];
    } catch (err) {
      forgeDebug({
        scope: 'webhooks-store',
        level: 'error',
        message:
          'listRoutesByAgent DB read failed: ' + (err instanceof Error ? err.message : String(err)),
      });
      return [];
    }
  }
  ...
  async function deactivateRoute(routeId: string): Promise<void> {
    try {
      await (db.update(webhookRoutes) as any)
      ...
      forgeDebug({
        scope: 'webhooks-store',
        level: 'error',
        message: 'deactivateRoute: db.update failed',
        context: { routeId, error: err instanceof Error ? err.message : String(err) },
      });
      throw err;
    }
  }
  ...
  async function createEvent(input: {
    ...
    try {
      ...
      forgeDebug({
        scope: 'webhooks-store',
        level: 'error',
        message: 'createEvent: db.insert failed',
        context: {
          routeId: input.routeId,
          agentId: input.agentId,
          error: err instanceof Error ? err.message : String(err),
  ...
  async function listEventsByAgent(agentId: string, limit = 50): Promise<WebhookEvent[]> {
    try {
      const _evRows = await db
        .select()
        .from(webhookEvents)
        .where(eq(webhookEvents.agentId, agentId))
        .orderBy(desc(webhookEvents.receivedAt))
        .limit(limit);
      return _evRows as unknown as any[] as WebhookEvent[];
    } catch (err) {
      forgeDebug({
        scope: 'webhooks-store',
        level: 'error',
        message:
          'listEventsByAgent DB read failed: ' + (err instanceof Error ? err.message : String(err)),
      });
      return [];
    }
  }
  ...
  async function markProcessed(eventId: string): Promise<void> {
    try {
      ...
      forgeDebug({
        scope: 'webhooks-store',
        level: 'error',
        message: 'markProcessed: db.update failed',
        context: { eventId, error: err instanceof Error ? err.message : String(err) },
      });
      throw err;
    }
  }
  ...
  async function markFailed(eventId: string): Promise<void> {
    try {
      ...
      forgeDebug({
        scope: 'webhooks-store',
        level: 'error',
        message: 'markFailed: db.update failed',
        context: { eventId, error: err instanceof Error ? err.message : String(err) },
      });
      throw err;
    }

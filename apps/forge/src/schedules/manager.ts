...
function createHeartbeatWakeInstruction(content?: string) {
  ...
  const lines = [
    ...
    `  Why: the workspace is your notebook and operating record. It contains context, reasoning, partial conclusions, follow-ups, and references that help you resume work with continuity instead of restarting from zero.`,
    `  What to do: read the files that matter for the current front, especially \`AGENT_CONTEXT.md\` and any deeper files it points to. Recover prior analysis, decisions, open questions, unfinished threads, and domain-specific notes, then decide what those records imply about what should happen next.`,
    '',
    'Phase 3. Clean and strengthen your records.',
    '- Refactor relevant workspace files.',
    '  Why: your workspace should become a useful operational knowledge base, not an unmanaged pile of fragments.',
    `  What to do: keep \`AGENT_CONTEXT.md\` compact, current, and high-signal; keep deeper detail in the other files it references; consolidate overlapping notes, organize material by topic or workstream, expand what deserves detail, refine conclusions, remove stale material, and leave the workspace easier to revisit later.`,
    '- Use documentation as analysis.',
    '  Why: documenting is not only archiving; it is also how you clarify thought, notice gaps, and turn vague intuition into actionable understanding.',
    '  What to do: while updating workspace files, explicitly refine what you believe, what you learned, what remains uncertain, and what concrete actions follow from that.',
    '- Avoid duplication.',
    ...
    '  What to do: if conversations, notifications, or workspace review reveal something that should be handled, move it forward instead of merely noting it.',
    ...
    '- If no clear pending item exists, deliberately enter reflection mode and search for useful work.',
    '  Why: lack of explicit assignment is not the same as lack of useful work, and many important things remain undone simply because nobody paused to think about them.',
    '  What to do: review your area and ask what is missing, neglected, drifting, weakly owned, insufficiently documented, risky, or obviously improvable. Look for initiatives, reviews, plans, discussions, validations, and improvements that should exist even if nobody has asked for them yet.',
    '- Turn reflection into concrete movement.',
    '  Why: reflection is only valuable if it produces better action.',
    '  What to do: when reflection reveals a useful direction, convert it into an explicit next step, record it where appropriate, and begin moving it instead of leaving it as a vague thought.',
  ];

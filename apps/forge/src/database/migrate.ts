...
export async function runMigrations(db: LibSQLDatabase<Record<string, unknown>>): Promise<void> {
  ...
  try {
    ...
    // Track applied migrations by hash, not by folderMillis. folderMillis values
    // in this project's journal are NOT monotonically increasing (e.g. journal
    // entry 24 has folderMillis 1780485628386 while entry 25 has 1780423781863
    // - the latter was created later in real time but its 'when' was generated
    // against an earlier baseline). Comparing by hash matches what drizzle's
    // own migrator does and is stable across re-runs.
    const appliedHashes = new Set<string>(
      Array.isArray(dbMigrations) ? dbMigrations.map((m) => m.hash) : []
    );

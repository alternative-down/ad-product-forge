...
import { createInternalChatGroups, type InternalChatGroups } from './internal-chat-groups';
...
function createMockDeps(accounts?: unknown[]) {
  ...
  return {
    ...
    getRequiredAgentAccount: vi.fn().mockImplementation((agentId: string) => {
      const found = store.find((a) => (a as Record<string, unknown>).agentId === agentId);
      if (!found) {
    ...
    getRequiredAccountBySlug: vi.fn().mockImplementation((slug: string) => {
      const found = store.find((a) => (a as Record<string, unknown>).slug === slug);
...
describe('createInternalChatGroups', () => {
  ...
  describe('changeChatGroup — update existing group', () => {
    beforeEach(() => {
      ...
      db.query.internalChatConversationMembers.findMany = vi.fn().mockResolvedValue([]);
    });
      ...
      const memberRows = [
        {
          groupId: 'grp_test_1',
          participantId: 'acc_agent_1',
          participantKey: 'agent_001',
          participantSlug: 'varek',
          participantName: 'Varek',
          role: 'admin',
          createdAt: 1710000000000,
        },
  ...
  describe('listGroupMembersByAccount', () => {
    ...
    it('returns members when account belongs to the group', async () => {
      ...
      const result = await groups.listGroupMembersByAccount({
        accountId: 'acc_agent_1',
        groupId: 'grp_test_1',
      });
  ...
  describe('requireConversationMembership', () => {
    ...
    it('throws when agent does not belong to conversation', async () => {
      ...
      await expect(groups.requireConversationMembership('agent_001', 'grp_test_1')).rejects.toThrow(
  ...
  describe('getRequiredGroupForAgent', () => {
    ...
    it('throws when conversation is a DM (not a group)', async () => {
      ...
      await expect(groups.getRequiredGroupForAgent('agent_001', 'dm_test_1')).rejects.toThrow(
        'Chat group not found: dm_test_1',
      );
    });
    ...
    it('throws when group does not exist', async () => {
      ...
      await expect(groups.getRequiredGroupForAgent('agent_001', 'nonexistent')).rejects.toThrow(
        'Conversation not found',
      );
    });
  });
  ...
  describe('getRequiredGroupForAccount', () => {
    ...
    it('throws when conversation is a DM (not a group)', async () => {
      ...
      await expect(groups.getRequiredGroupForAccount('acc_agent_1', 'dm_test_1')).rejects.toThrow(
        'Chat group not found: dm_test_1',
      );
    });
    ...
    it('throws when group does not exist', async () => {
      ...
      await expect(groups.getRequiredGroupForAccount('acc_agent_1', 'nonexistent')).rejects.toThrow(
        'Conversation not found',
      );
    });
  });

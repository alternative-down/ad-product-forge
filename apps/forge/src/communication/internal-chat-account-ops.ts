...
export interface InternalChatAccountOpsDeps {
  ...
  listGroupMembersByAccount(input: { accountId: string; groupId: string }): Promise<
    Array<{
...
export function createInternalChatAccountOps(db: Database, deps: InternalChatAccountOpsDeps) {
  ...
  async function createExternalChatGroupWithMembers(input: {
    ...
    const groupMembers = await db.transaction(async (tx) => {
      ...
      const membersToAdd = memberAccounts.map((a) => a.id).filter((id) => id !== creatorId);
  ...
  async function ensureDirectConversationByAccount(input: {
    ...
    const conversation = await deps.ensureDirectConversation(
      input.accountId,
      input.participantAccountId,
    );
    if (conversation === null || conversation === undefined) {
      forgeDebug({
        scope: 'internal-chat-account-ops',
        level: 'error',

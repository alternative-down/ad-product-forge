...
export function createChatSending(deps: SendingDeps) {
  ...
  async function sendMessage(input: {
    ...
    if (!conversation) {
      forgeDebug({ scope: 'internal-chat-sending', level: 'warn', message: 'sendInternalChatMessage: conversation not found', context: { targetKey: input.targetKey } });
      forgeDebug({ scope: 'internal-chat-sending', level: 'warn', message: 'sendMessage: conversation not found', context: { targetKey: input.targetKey } });

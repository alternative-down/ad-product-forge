  ...
  if (credentials.discord) {
    try {
      ...
      try {
        ...
        forgeDebug({ scope: "communication-provider-loader.ts", level: "error", message: "communication-provider-loader.ts: unhandled error", error: err instanceof Error ? err.message : String(err) });
        throw error;
      }

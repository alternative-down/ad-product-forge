...
async function main() {
  ...
  forgeDebug({ scope: 'top-up-contract', level: 'info', message: 'Top-up applied', context: { agentId: result.agentId, contractId: result.contractId, budgetUsd: result.budgetUsd } });
...
main().catch((error) => {
  forgeDebug({ scope: 'top-up-contract', level: 'error', message: 'Failed to top up active contract', context: { error: String(serializeError(error)) } });
  process.exit(1);
});

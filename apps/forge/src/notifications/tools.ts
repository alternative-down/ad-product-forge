    ...
    tools.list_agent_notifications = createTool({
      ...
      execute: async (input) => {
        try {
          ...
          forgeDebug({ scope: 'notifications-tools', level: 'error', message: '[notifications-tools] list_agent_notifications failed', context: { error: error instanceof Error ? error.message : String(error) }});
          return {
            valid: false,

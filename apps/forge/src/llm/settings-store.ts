...
export function createLlmSettingsStore(db: Database) {
  ...
  async function updateDefaults(input: {
    ...
    if (existing) {
      ...
      try {
        ...
        forgeDebug({ scope: 'llm', level: 'info', message: 'Failed to insert LLM defaults', context: { error: err instanceof Error ? err.message : String(err) });
        throw err;
      }
    }
  ...
  async function getDefaultsRow() {
    try {
      ...
      forgeDebug({ scope: 'llm', level: 'info', message: 'Failed to get LLM defaults row', context: { error: err instanceof Error ? err.message : String(err) });
      throw err;
    }
  }

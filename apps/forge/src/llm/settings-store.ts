...
export function createLlmSettingsStore(db: Database) {
  ...
  async function upsertProfile(input: {
    ...
    try {
      ...
      forgeDebug({ scope: 'llm-settings', level: 'info', message: 'Failed to query LLM profile', context: { profileId, error: err instanceof Error ? err.message : String(err) } });
      throw err;
    }
    ...
    if (existing) {
      ...
      try {
        ...
        forgeDebug({ scope: 'llm-settings', level: 'info', message: 'Failed to insert LLM profile', context: { profileId, error: err instanceof Error ? err.message : String(err) } });
        throw err;
      }
    }
  ...
  async function updateDefaults(input: {
    ...
    try {
      ...
      forgeDebug({ scope: 'llm-settings', level: 'info', message: 'Failed to query LLM defaults', context: { error: err instanceof Error ? err.message : String(err) } });
      throw err;
    }

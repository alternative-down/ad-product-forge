...
export function createAgentReadModel(deps: AgentsReadModelDeps): AgentReadModel {
  ...
  async function getAgentRuntimeMemory(agentId: string) {
    ...
    try {
      ...
      try {
        agentContext = (await readFile(agentContextPath, 'utf8')).trim() ?? null;
      } catch (err) {
  ...
  async function debugAgentLongTermMemoryRecallSearch(
    ...
    const loadedAgent = registry.get(agentId);
    if (!loadedAgent) {
      forgeDebug({ scope: 'admin-read-model-agents', level: 'warn', message: 'getAgentRuntimeStatus: agent not loaded', context: { agentId } });
      throw new Error(`Agent is not loaded: ${agentId}`);
    }
    if (!loadedAgent.runtime.longTermMemoryRecall) {
      forgeDebug({ scope: 'admin-read-model-agents', level: 'warn', message: 'getAgentRuntimeStatus: LTM recall not available', context: { agentId } });

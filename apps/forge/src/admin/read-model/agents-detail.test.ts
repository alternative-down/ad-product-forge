...
const mockToScheduleSummary = vi.hoisted(() => {
  const fn = vi.fn();
  fn.mockReturnValue({ id: 's1', nextTriggerAt: 0, cron: '0 * * * *', enabled: true, lastTriggerAt: null });
  return fn;
});
...
describe('createAgentDetailReadModel', () => {
  beforeEach(() => {
    ...
    mockToScheduleSummary.mockReturnValue({ id: 's1', nextTriggerAt: 0, cron: '0 * * * *', enabled: true, lastTriggerAt: null });
  });

  describe('listAgentContracts', () => {
    it('returns empty array when no contracts exist', async () => {
      const db = makeMockDb({ query: { agentExecutionContracts: { findMany: vi.fn().mockResolvedValue([]) } } });
    ...
    it('maps id to contractId and strips id', async () => {
      ...
      const db = makeMockDb({ query: { agentExecutionContracts: { findMany: vi.fn().mockResolvedValue(contracts) } } });
      const model = createAgentDetailReadModel({ db });
    ...
    it('throws and logs forgeDebug on DB error', async () => {
      const db = makeMockDb({ query: { agentExecutionContracts: { findMany: vi.fn().mockRejectedValue(new Error('DB failed')) } } });
      const model = createAgentDetailReadModel({ db });
      await expect(model.listAgentContracts('agent-1')).rejects.toThrow('DB failed');
      expect(mockForgeDebug).toHaveBeenCalledWith(expect.objectContaining({ level: 'error', message: expect.stringContaining('listAgentContracts') }));
    });
  });
  ...
  describe('listAgentSchedules', () => {
    ...
    it('maps rows through toScheduleSummary', async () => {
      ...
      const db = makeMockDb({ query: { agentSchedules: { findMany: vi.fn().mockResolvedValue(rows) } } });
      const model = createAgentDetailReadModel({ db });
    ...
    it('logs forgeDebug on DB error', async () => {
      const db = makeMockDb({ query: { agentSchedules: { findMany: vi.fn().mockRejectedValue(new Error('read error')) } } });
      const model = createAgentDetailReadModel({ db });
      await expect(model.listAgentSchedules('agent-1')).rejects.toThrow('read error');
      expect(mockForgeDebug).toHaveBeenCalledWith(expect.objectContaining({ level: 'error', message: expect.stringContaining('listAgentSchedules') }));
    });
  });
  ...
  describe('listAgentNotifications', () => {
    ...
    it('maps notification fields (id, content, createdAt, readAt)', async () => {
      ...
      const db = makeMockDb({ query: { agentNotifications: { findMany: vi.fn().mockResolvedValue(notifications) } } });
      const model = createAgentDetailReadModel({ db });
    ...
    it('passes limit: 50 and desc orderBy to findMany', async () => {
      ...
      const db = makeMockDb({ query: { agentNotifications: { findMany: vi.fn().mockImplementation((opts: Record<string, unknown>) => { captured = opts; return []; }) } } });
      const model = createAgentDetailReadModel({ db });
    ...
    it('logs forgeDebug on DB error', async () => {
      const db = makeMockDb({ query: { agentNotifications: { findMany: vi.fn().mockRejectedValue(new Error('read fail')) } } });
      const model = createAgentDetailReadModel({ db });
      await expect(model.listAgentNotifications('agent-1')).rejects.toThrow('read fail');
      expect(mockForgeDebug).toHaveBeenCalledWith(expect.objectContaining({ level: 'error', message: expect.stringContaining('listAgentNotifications') }));
    });
  });
  ...
  describe('listAgentMcpServers', () => {
    ...
    it('merges agentMcpConfigs with mcpServerConfigs', async () => {
      const configs = [{ id: 'link-1', serverId: 'srv-1', isActive: 1 }, { id: 'link-2', serverId: 'srv-2', isActive: 0 }];
      const servers = [
        { id: 'srv-1', name: 'S1', description: 'd1', transport: 'stdio' as const, command: 'node', args: '[]', envVars: '', url: '', headers: '', createdAt: 111, updatedAt: 222 },
        { id: 'srv-2', name: 'S2', description: null, transport: 'http_streamable' as const, command: null, args: null, envVars: null, url: null, headers: null, createdAt: 333, updatedAt: 444 },
      ];
      const db = makeMockDb({ query: { agentMcpConfigs: { findMany: vi.fn().mockResolvedValue(configs) }, mcpServerConfigs: { findMany: vi.fn().mockResolvedValue(servers) } } });
      const model = createAgentDetailReadModel({ db });
      const result = await model.listAgentMcpServers('agent-1');
      expect(result).toHaveLength(2);
      expect(result[0]).toEqual({ configId: 'link-1', serverId: 'srv-1', name: 'S1', description: 'd1', transport: 'stdio', command: 'node', argsText: '[]', envVarsText: '', url: '', headersText: '', isActive: true, createdAt: 111, updatedAt: 222 });
    ...
    it('skips configs with no matching server', async () => {
      ...
      const servers = [{ id: 'srv-other', name: 'Other', description: null, transport: 'stdio' as const, command: null, args: null, envVars: null, url: null, headers: null, createdAt: 0, updatedAt: 0 }];
      const db = makeMockDb({ query: { agentMcpConfigs: { findMany: vi.fn().mockResolvedValue(configs) }, mcpServerConfigs: { findMany: vi.fn().mockResolvedValue(servers) } } });
      const model = createAgentDetailReadModel({ db });
      const result = await model.listAgentMcpServers('agent-1');
    ...
    it('logs forgeDebug when mcpConfigs read fails', async () => {
      const db = makeMockDb({ query: { agentMcpConfigs: { findMany: vi.fn().mockRejectedValue(new Error('config fail')) } } });
      const model = createAgentDetailReadModel({ db });
      await expect(model.listAgentMcpServers('agent-1')).rejects.toThrow('config fail');
      expect(mockForgeDebug).toHaveBeenCalledWith(expect.objectContaining({ level: 'error', message: expect.stringContaining('listAgentMcpServers') }));
    });

    it('logs forgeDebug when serverConfigs read fails', async () => {
      const configs = [{ id: 'link-1', serverId: 'srv-1', isActive: 1 }];
      const db = makeMockDb({ query: { agentMcpConfigs: { findMany: vi.fn().mockResolvedValue(configs) }, mcpServerConfigs: { findMany: vi.fn().mockRejectedValue(new Error('server fail')) } } });
      const model = createAgentDetailReadModel({ db });
  ...
  describe('listAgentLlmProfiles', () => {
    ...
    it('returns empty profiles when agent has no profile IDs', async () => {
      const db = makeMockDb({ query: { agents: { findFirst: vi.fn().mockResolvedValue({ modelProfileId: null, omModelProfileId: null }) } } });
      const model = createAgentDetailReadModel({ db });
      expect(await model.listAgentLlmProfiles('agent-1')).toEqual({ profiles: [] });
    });

    it('returns profiles when agent has profile IDs', async () => {
      const profiles = [{ id: 'prof-1', name: 'GPT-4' }, { id: 'prof-2', name: 'Claude' }];
      const db = makeMockDb({ query: { agents: { findFirst: vi.fn().mockResolvedValue({ modelProfileId: 'prof-1', omModelProfileId: 'prof-2' }) }, llmProfiles: { findMany: vi.fn().mockResolvedValue(profiles) } } });
      const model = createAgentDetailReadModel({ db });
      expect(await model.listAgentLlmProfiles('agent-1')).toEqual({ profiles });
    });

    it('logs forgeDebug on agent read error', async () => {
      const db = makeMockDb({ query: { agents: { findFirst: vi.fn().mockRejectedValue(new Error('agent read fail')) } } });
      const model = createAgentDetailReadModel({ db });
      await expect(model.listAgentLlmProfiles('agent-1')).rejects.toThrow('agent read fail');
      expect(mockForgeDebug).toHaveBeenCalledWith(expect.objectContaining({ level: 'error', message: expect.stringContaining('listAgentLlmProfiles') }));
    });

    it('logs forgeDebug on profiles read error', async () => {
      const db = makeMockDb({ query: { agents: { findFirst: vi.fn().mockResolvedValue({ modelProfileId: 'prof-1', omModelProfileId: null }) }, llmProfiles: { findMany: vi.fn().mockRejectedValue(new Error('profile fail')) } } });
      const model = createAgentDetailReadModel({ db });

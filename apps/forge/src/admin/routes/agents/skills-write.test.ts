...
const { mockUploadSchema, mockDeleteSchema, mockInstallGlobalSchema, mockPublishSchema } =
  vi.hoisted(() => ({
    mockUploadSchema: {
      parse: vi.fn((x) => x),
      safeParse: vi.fn((x) => ({ success: true, data: x })),
    },
    mockDeleteSchema: {
      parse: vi.fn((x) => x),
      safeParse: vi.fn((x) => ({ success: true, data: x })),
    },
    mockInstallGlobalSchema: {
      parse: vi.fn((x) => x),
      safeParse: vi.fn((x) => ({ success: true, data: x })),
    },
    mockPublishSchema: {
...
describe('registerAgentSkillsWriteRoutes', () => {
  beforeEach(() => {
    ...
    mockParseJsonBody.mockImplementation(
      (text: string, schema: { parse: (x: unknown) => unknown }) => schema.parse(JSON.parse(text)),
    );
  ...
  describe('route registration', () => {
    it('registers POST /admin/agent-skills/upload', () => {
      ...
      registerAgentSkillsWriteRoutes(httpServer, {
        db: mockDb(makeAgent()),
        loaderConfig: {
          githubApps: {} as unknown,
          emailMailboxes: {} as unknown,
          coolify: {} as unknown,
        } as unknown,
    ...
    it('registers POST /admin/agent-skills/delete', () => {
      ...
      expect(
        httpServer.getRoutes().find((r) => r.path === '/admin/agent-skills/delete'),
      ).toBeDefined();
    });
    ...
    it('registers POST /admin/agent-skills/install-global', () => {
      ...
      expect(
        httpServer.getRoutes().find((r) => r.path === '/admin/agent-skills/install-global'),
      ).toBeDefined();
    });
    ...
    it('registers POST /admin/agent-skills/publish-global', () => {
      ...
      expect(
        httpServer.getRoutes().find((r) => r.path === '/admin/agent-skills/publish-global'),
      ).toBeDefined();
    });
  ...
  describe('POST /admin/agent-skills/upload', () => {
    it('returns 404 when agent not found', async () => {
      ...
      const response = await route.handler(
        makeRequest({ agentId: 'unknown', archiveBase64: 'YQ==' }),
      );
    ...
    it('calls installAgentWorkspaceSkillsFromZip with correct args', async () => {
      ...
      const route = httpServer.getRoutes().find((r) => r.path === '/admin/agent-skills/upload')!;
      await route.handler(
        makeRequest({ agentId: 'agent-upload', archiveBase64: 'SGVsbG8gV29ybGQ=' }),
      );
      expect(mockInstallZip).toHaveBeenCalledWith(
    ...
    it('returns 201 with installedSkillNames on success', async () => {
      ...
      const route = httpServer.getRoutes().find((r) => r.path === '/admin/agent-skills/upload')!;
      const response = await route.handler(
        makeRequest({ agentId: 'agent-1', archiveBase64: 'YQ==' }),
      );
  ...
  describe('POST /admin/agent-skills/delete', () => {
    ...
    it('returns 200 with success and skillName on success', async () => {
      ...
      const route = httpServer.getRoutes().find((r) => r.path === '/admin/agent-skills/delete')!;
      const response = await route.handler(
        makeRequest({ agentId: 'agent-1', skillName: 'obsolete' }),
      );
  ...
  describe('POST /admin/agent-skills/install-global', () => {
    it('returns 404 when agent not found', async () => {
      ...
      const route = httpServer
        .getRoutes()
        .find((r) => r.path === '/admin/agent-skills/install-global')!;
      const response = await route.handler(makeRequest({ agentId: 'unknown', skillName: 'x' }));
      expect(response.status).toBe(404);
    });
    ...
    it('calls installGlobalSkillToAgentWorkspace with correct args', async () => {
      ...
      expect(mockInstallGlobal).toHaveBeenCalledWith(
        expect.objectContaining({
          workspaceBasePath: '/ws',
          agent,
    ...
    it('returns 200 with success on install', async () => {
      ...
      const route = httpServer
        ...
        .find((r) => r.path === '/admin/agent-skills/install-global')!;
      const response = await route.handler(
        makeRequest({ agentId: 'agent-1', skillName: 'fetch-data' }),
      );
      expect(response.status).toBe(200);
      expect(parseBody(response)).toMatchObject({
        success: true,
  ...
  describe('POST /admin/agent-skills/publish-global', () => {
    it('returns 404 when agent not found', async () => {
      ...
      const route = httpServer
        .getRoutes()
        .find((r) => r.path === '/admin/agent-skills/publish-global')!;
      const response = await route.handler(makeRequest({ agentId: 'unknown', skillName: 'x' }));
      expect(response.status).toBe(404);
    });
    ...
    it('calls publishAgentWorkspaceSkillToGlobalCatalog with correct args', async () => {
      ...
      expect(mockPublishGlobal).toHaveBeenCalledWith(
        expect.objectContaining({
          workspaceBasePath: '/ws',
          agent,
          skillName: 'my-skill',

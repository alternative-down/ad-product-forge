...
function buildSystemLocation(input: {
  ...
  | { to: '/v1/system/oauth' }
  | { to: '/v1/system/integrations/migadu' }
  | { to: '/v1/system/integrations/coolify' }
  | { to: '/v1/system/integrations/github' }
  | { to: '/v1/system/integrations/minimax' }
  | { to: '/v1/system/migrations' } {
  if (input.section === 'company') {
    return { to: '/v1/system/company' };
  }

  if (input.section === 'llm') {
    if (input.llmView === 'profiles') {
      return { to: '/v1/system/llm/profiles' };
    }

    if (input.llmView === 'prices') {
      return { to: '/v1/system/llm/prices' };
    }

    return { to: '/v1/system/llm/defaults' };
  }

  if (input.section === 'auth') {
    return { to: '/v1/system/oauth' };
  }

  if (input.section === 'integrations') {
    if (input.integrationView === 'coolify') {
      return { to: '/v1/system/integrations/coolify' };
    }

    if (input.integrationView === 'github') {
      return { to: '/v1/system/integrations/github' };
    }

    if (input.integrationView === 'minimax') {
      return { to: '/v1/system/integrations/minimax' };
    }

    return { to: '/v1/system/integrations/migadu' };
  }

  return { to: '/v1/system/migrations' };
}

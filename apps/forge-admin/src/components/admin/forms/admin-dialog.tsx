...
export function AdminDialogContent({ className, ...props }: ComponentProps<typeof DialogContent>) {
  return (
    <DialogContent
      className={cn(
        'flex h-[calc(100dvh-2rem)] flex-col overflow-hidden rounded-lg',
        className,
      )}
...
export function AdminDialogBody({ className, children, ...props }: ComponentProps<'div'>) {
  return (
    <AdminScrollArea className="max-h-[min(30rem,calc(100dvh-14rem))]" contentClassName={cn('space-y-4 py-4 pl-px pr-4', className)}>
      <div {...props}>
        {children}
      </div>
    </AdminScrollArea>

...
type ListConversationsInput = z.infer<typeof listConversationsInputSchema>;
...
export function createListConversationsTool(communication: CommunicationModule) {
  ...
  const createCommunicationTool = createTool as unknown as (input: {
    id: string;
    description: string;
    inputSchema: z.ZodTypeAny;
    execute: (input: ListConversationsInput) => Promise<unknown>;
  }) => unknown;

  return createCommunicationTool({

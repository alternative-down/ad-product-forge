...
type MarketResearchInput = z.infer<typeof marketResearchInputSchema>;

const createMarketResearchTool = createTool as unknown as (input: {
  id: string;
  description: string;
  inputSchema: z.ZodTypeAny;
  execute: (input: MarketResearchInput) => Promise<unknown>;
}) => unknown;

export const marketResearchTool = createMarketResearchTool({
  id: 'market_research',
  description:
...
Focus on authentic signals from real users, not marketing hype. Prioritize signals that suggest real market demand.`;
    ...
    try {
      const rawResponse = await firecrawl.agent({
        prompt,
        schema: signalSchema as unknown as Record<string, unknown>,
      });

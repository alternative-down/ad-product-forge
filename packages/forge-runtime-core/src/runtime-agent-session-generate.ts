...
function expandRecentReplayMessages(input: {
  ...
  const selectedMessageIds = new Set(input.recentMessageIds);
  const toolCallMessageIdsByCallId = new Map<string, string>();
  const toolResultMessageIdsByCallId = new Map<string, string>();
  ...
  for (const message of input.activeMessages) {
    if (message.role === 'assistant') {
      ...
      for (const toolInvocation of toolInvocations) {
        if (
          ...
          toolCallMessageIdsByCallId.set(toolInvocation.toolCallId, message.id);
        }
      }
    ...
    for (const toolResult of toolResults) {
      if (
        ...
        toolResultMessageIdsByCallId.set(toolResult.toolCallId, message.id);
      }
    }
  }

  for (const messageId of [...selectedMessageIds]) {
    const message = input.activeMessageMap.get(messageId);
    ...
    if (message.role === 'assistant') {
      ...
      for (const toolInvocation of toolInvocations) {
        ...
        const toolResultMessageId = toolResultMessageIdsByCallId.get(toolInvocation.toolCallId);

        if (toolResultMessageId) {
          selectedMessageIds.add(toolResultMessageId);
        }
      }
    ...
    for (const toolResult of toolResults) {
      ...
      const toolCallMessageId = toolCallMessageIdsByCallId.get(toolResult.toolCallId);

      if (toolCallMessageId) {
        selectedMessageIds.add(toolCallMessageId);
      }
    }
  }

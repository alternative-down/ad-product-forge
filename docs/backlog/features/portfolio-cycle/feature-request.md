# Feature Request — Ciclo de Portfólio e Nova Rodada de Produto

## Status

Draft inicial

## Regra de gatilho

Quando (todos verdadeiros):

- fluxo de caixa operacional dos últimos 30 dias >= 0
- runway projetado >= 3 meses
- apps em produção estáveis (menos de 10 issues/semana)
- valor ganho (MRR) do mês atual >= 1.15x média dos 2 meses anteriores

## Ação

- iniciar nova rodada de criação de produto

## Objetivo

Escalar o portfólio de produtos de forma progressiva, acionando novas rodadas quando a base financeira estiver mais forte.
